...
const SectionKicker = ({ children, className = "", style = {} }) => (
  <div className={`text-uppercase text-muted ${className}`} style={{ fontSize: 11, letterSpacing: 0.6, ...style }}>
    {children}
  </div>
);
...
class RosterShow extends Component {
  ...
  loadPage = () => {
    ...
    Promise.all([api.get(`/rosters/${id}`), api.get(`/students/all`), api.get(`/users?role=teacher`)])
      .then(([rosterRes, studentsRes, teachersRes]) => {
        ...
        const allStudents = Array.isArray(sd)
          ? sd
  ...
  submitMeeting = (e) => {
    ...
    api
      .post(`/rosters/${id}/roster_meetings`, {
        roster_meeting: {
          taught_on,
          starts_at,
          ends_at,
  ...
  fmtTimeAny = (value) => {
    ...
    const looksLikeTimeOnlyISO =
      typeof value === "string" && (/^2000-01-01T/.test(value) || /^1970-01-01T/.test(value) || value.endsWith("Z"));
      ...
      <div className="container mt-4" style={{ maxWidth: 1100 }}>
        ...
        <div className="d-flex flex-column flex-sm-row justify-content-between align-items-stretch align-items-sm-start gap-3 mb-3">
          ...
          <div className="d-flex flex-column flex-sm-row gap-2">
            {!this.state.isEditingTitle ? (
              <Button size="sm" variant="primary" onClick={this.startEditTitle} {...pillBtn}>
                Edit title
              </Button>
            ) : (
              <>
                <Button size="sm" variant="primary" onClick={this.saveTitle} disabled={this.state.titleSaving} {...pillBtn}>
            ...
            <Card style={{ borderRadius: 14 }} className="h-100">
              <Card.Body>
                ...
                <div className="mt-3">
                  ...
                  {sq && (
                    <div className="border rounded-3 mt-2" style={{ maxHeight: 240, overflowY: "auto", background: "#fff" }}>
                      {addMatches.map((s) => (
                        <button
                          key={s.id}
                          type="button"
                  ...
                  <Form onSubmit={this.createWeeklySchedule}>
                    <Row className="g-2">
                      <Col md={12}>
                        ...
                        <Form.Select value={this.state.newWeekday} onChange={(e) => this.setState({ newWeekday: e.target.value })}>
                      ...
                      <Col xs={6}>
                        ...
                        <Form.Control type="time" value={this.state.newStart} onChange={(e) => this.setState({ newStart: e.target.value })} required />
                      </Col>

                      <Col xs={6}>
                        <Form.Label>End</Form.Label>
                        <Form.Control type="time" value={this.state.newEnd} onChange={(e) => this.setState({ newEnd: e.target.value })} required />
                      </Col>
                      ...
                      <Col md={12} className="mt-2">
                        <Button type="submit" className="btn btn-sm rounded-pill px-3" style={{ fontSize: 12, minWidth: 84 }} disabled={this.state.saving}>
                          Add weekly time
                        </Button>
                      </Col>
                    </Row>
                ...
                <div className="border rounded-3 p-3">
                  ...
                  <Form onSubmit={this.submitMeeting}>
                    <Row className="g-2">
                      ...
                      <Col md={12} className="mt-2">
                        <Button type="submit" className="btn btn-sm rounded-pill px-3" style={{ fontSize: 12, minWidth: 84 }} disabled={this.state.meetingSaving}>
                  ...
                  <div className="form-text mt-2">One-off meetings are specific dates (not weekly repeating).</div>
                </div>
              </Col>
            </Row>
          </Card.Body>
